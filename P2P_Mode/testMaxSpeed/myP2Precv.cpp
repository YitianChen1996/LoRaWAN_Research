...
char crc_mode[] = "on";
int recv_timeout, prenum = 0, precnt = 0;
char temp2[3], temp1[260];
//////////////////////////////////////////////
...
void loop() 
{
  ...
  printf("\nListening to packets...\n");
   ...
   // rx
  error = LoRaWAN.receiveRadio(recv_timeout);
  
  // Check status
  if (error == 0)
  {
    printf("--> Packet received No.%d",cnt);
    cnt++;
    printf("packet: %s\n", (char*) LoRaWAN._buffer);
    if (LoRaWAN._length==2){
    	ofstream datafile;
    ...
    //strcpy(temp1, (char*)LoRaWAN._buffer);
    //strcpy(temp2,temp1+strlen(temp1)-2);
    //strcpy(temp,*(((char*)LoRaWAN._buffer)+LoRaWAN._length-2));
    //int len = LoRaWAN._length;
    //int num = atoi(temp2);
    //cout<<"num:"<<num<<endl;
